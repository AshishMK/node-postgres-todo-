var telesign = require('telesign').setup({
  ...
  apiKey: 'mOehrIarTb0w4ae/+CHIG/rTZRHb+DsdzkROZEZrZFuFu+IvyHGzyfzae6VgJ9A7OUzm$
});
...
var p = plivo.RestAPI({
  authId: 'AODQ1NTI5NDY4NTY4ZJ',
  authToken: 'OWQ0OTQ4NDRlNzFiODRmYWEzZWRhM2Q3MDk0Y2E2'
});
...
/* GET home page. */
router.get('/verifySmsOD/:phone',function (req, res, next)  {
...
var options = {
  ...
  template: 'Your code is '+num // optional, must include a $$CODE$$ placeholder $
};
...
console.log("started ccccc");
  pg.connect(connectionString, function(err, client, done) {
   ...
   client.query(
            'insert into what_status_views (uid,status_id,name,phone,vid,created_at) values($1,$2,$3,$4,$5,now())',
            [uid,status_id,name,phone,vid],
            function(err, result) {
                if (err) {
                    console.log(err);
 return res.status(500).json({success: false, data: err});
...
console.log("started ccccc");
  pg.connect(connectionString, function(err, client, done) {
   ...
   client.query(
            'delete from what_status where _id=$1',
            [status_id],
            function(err, result) {
                if (err) {
                    console.log(err);
...
var text1=req.body.text1;
  ...
  pg.connect(connectionString, function(err, client, done) {
   ...
   client.query(
            'insert into what_status (uid,text1,created_at) values($1,$2,now())',
            [uid,text1],
            function(err, result) {
                if (err) {
                    console.log(err);
...
var uid=req.params.uid;
 ...
 query = client.query("select *  from what_status_views where uid ='"+uid+"' and created_at >now() - interval '24 hours'  order by created_at desc");
