var express = require('express'); var multer = require('multer'); var fs = require('fs'); var path = require('path'); var os = require('os'); var passport = 
require('passport'); var HttpBasicAuth = require('passport-http').BasicStrategy; var app = express(); var UPLOAD_PATH = "./uploads/"; var SERVER_PORT = 3000; 
var basicAuthUser = {
    username: "test",
    password: "pass"
};
...
// configure passport for Basic Auth strategy passport.use('basic-admin', new HttpBasicAuth({ realm: 'Upload Service' },
...
)); app.use(passport.initialize()); var useBasicAuth = passport.authenticate('basic-admin', { session: false }); // configure multer for upload management var 
fileUploadCompleted = false; var multerFiles = multer({
    dest: UPLOAD_PATH,
...
// handle multipart uploads app.post('/upload/multipart', multipartReqInterceptor, multerFiles, multipartUploadHandler); app.post('/upload/multipart-ba', 
useBasicAuth, multipartReqInterceptor, multerFiles, multipartUploadHandler); // handle binary uploads app.post('/upload/binary', binaryUploadHandler); 
app.post('/upload/binary-ba', useBasicAuth,binaryUploadHandler); // endpoint which returns always 401 and a JSON response in the body 
